...
#ggplot2::ggsave(here::here('data/figures/volcanoes/volcanoes_all.pdf'), height=50, width=180, units = "mm")
ggplot2::ggsave(here::here('data/figures/limma/volcanoes/volcanoes_limma.png'), height=75, width=200, units = "mm")
...
ggplot2::ggsave(here::here('data/figures/euler/uptake.pdf'), height=150, width=150, units = "mm", plot = euler_uptake_plot)
...
ggplot2::ggsave(here::here('data/figures/euler/uptake.pdf'), height=150, width=150, units = "mm", plot = euler_release_plot)
```

List of released metabolites shared among exercise and recovery
```{r}
#Exercise and rec60
Reduce(intersect, list(dplyr::filter(results_exercise, xiao <0.05 & logFC < 0)$metabolite,
                       dplyr::filter(results_rec60, xiao <0.05 & logFC < 0)$metabolite))
...
results_combined <- rbind(results_baseline, results_exercise, results_rec60, results_rec180)

df_long_av_difference_summary %>% 
    merge(results_combined[,c("time", "metabolite", "regulated_xiao")], by=c("time", "metabolite")) %>% #Merge the "regulated" column into results_combined
    dplyr::filter(regulated_xiao == "+") %>% #Filter for only significant metabolites
    dplyr::mutate(direction = ifelse(mean > 0, "Uptake", "Release")) %>% #Creates a new column for color coding the plot
    dplyr::mutate(mean = abs(mean)) %>% #Makes all values positive
    dplyr::mutate(time = factor(time, levels = c("pre", "baseline", "exercise", "rec60", "rec180"))) %>% 
    dplyr::filter(!mean == 0) %>% 
...
ggplot2::ggsave(here::here('data/figures/limma/number_of_release_uptake.png'), height=100, width=200, units = "mm")
